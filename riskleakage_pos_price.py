...
def analyze_intraday_leakage_continuous(
    ...
    # Unit price per trade (EUR): |premium_EUR| × futurePointValue.
    df["unit_price"] = df["premium_eur"].abs() * df["futurePointValue_numeric"]

    # Separate buy/sell prices for direction-specific reference prices.
    is_buy = df["way"].str.upper() == "BUY"
    df["buy_price"] = np.where(is_buy, df["unit_price"], np.nan)
    df["sell_price"] = np.where(~is_buy, df["unit_price"], np.nan)

    # Reference prices: expanding mean per position group, forward-filled.
    df = df.sort_values(position_keys + ["hour_bucket"], kind="mergesort").reset_index(drop=True)
    df["avg_buy_price"] = df.groupby(position_keys)["buy_price"].transform(
        lambda s: s.expanding().mean()
    )
    df["avg_sell_price"] = df.groupby(position_keys)["sell_price"].transform(
        lambda s: s.expanding().mean()
    )
    df["avg_price"] = df.groupby(position_keys)["unit_price"].transform(
        lambda s: s.expanding().mean()
    )
    df["avg_buy_price"] = df.groupby(position_keys)["avg_buy_price"].ffill()
    df["avg_sell_price"] = df.groupby(position_keys)["avg_sell_price"].ffill()
    df["avg_price"] = df.groupby(position_keys)["avg_price"].ffill()

    # Nominal per trade using reference average price (quantity × avg_price).
    df["nominal"] = df["quantity"] * df["avg_price"]
    df["signed_qty"] = np.where(is_buy, df["quantity"], -df["quantity"])
    df["signed_nominal"] = np.where(is_buy, df["nominal"], -df["nominal"])
    df["buy_nominal"] = np.where(is_buy, df["quantity"] * df["avg_buy_price"], 0.0)
    df["sell_nominal"] = np.where(~is_buy, df["quantity"] * df["avg_sell_price"], 0.0)
    ...
    # Forward fill avg_price for continuity across hours.
    hourly_net["avg_price"] = hourly_net.groupby(position_keys)["avg_price"].ffill()

    # 4) Rebuild position path from hourly net quantity flow.
    hourly_net["cumulative_position"] = hourly_net.groupby(position_keys)[
        "signed_qty"
    ].cumsum()
    ...
    for group_ids, group_df in hourly_net.groupby(daily_keys):
        ...
        sod_position = group_df["cumulative_position"].iloc[0]
        eod_position = group_df["cumulative_position"].iloc[-1]
        # Position entering the day before any first-hour trading.
        prior_eod_position = sod_position - group_df["signed_qty"].iloc[0]

        # Convert position to nominal using reference average price.
        ref_price = group_df["avg_price"].ffill().bfill().iloc[-1]
        sod_nominal = sod_position * ref_price
        eod_nominal = eod_position * ref_price
        prior_eod_nominal = prior_eod_position * ref_price

        max_exposure = group_df["cumulative_position"].abs().max() * ref_price
        ...
        max_nominal_raw = group_df["cumulative_position"].max() * ref_price
        baseline_deviation_ratio = (
    ...
    if not leakage_df.empty and plot_top_pct > 0:
        ...
        for _, row in to_plot_df.iterrows():
            ...
            # Derive nominal from position × reference price for plotting.
            group_df["cumulative_nominal"] = group_df["cumulative_position"] * group_df["avg_price"].ffill().bfill()
